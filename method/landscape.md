<!-- GENERATED by scripts/publish-public.sh from toolkit/references/landscape.md. Do not edit here. -->
...
(`method/SKILL.md`, `method/{lenses,aimm-checklist,properties}.md`,
`~/.claude/skills/solidity-audit/{SKILL.md,checklists/01..21,references/*}`, ~1490 lines).

**Headline:** 197 distinct sources registered (196 at sweep + `CDSecurity/cdsecurity-skills`, post-sweep 2026-09-10). 92 repositories were cloned and read as source at a
...
| CDSecurity/cdsecurity-skills | github.com/CDSecurity/cdsecurity-skills | `930cc41` (clone, post-sweep 2026-09-10) | 2 prep skills v1.0: `audit-prep` (Solidity 8-phase) + `rust-audit-prep` (Solana 5-phase). Explicitly NOT bug hunters | RESIDUE process-only | prep gates → `21/22`, CQ-27..38, TC-18..22, DV-1..6; Rust hygiene noted for Rust mirror; Solana items N/A |
...
| OWASP Smart Contract Top 10 (2025) | owasp.org/www-project-smart-contract-top-10 | fetched 2026-09-09 | ten-item awareness list; 2025 promotes Access Control to #1 and Price Oracle Manipulation to #2 | DUPLICATE, zero orphans (S9) | S9 mapping table SC01..SC10; the only structural gaps are SC03 Logic Errors and SC04 Input Validation, which have no owning file in `checklists/01..21` |
...
Sharply separated: what was **executed on this box** with a number, versus what was **read about**.

Host for all S5 runs: macOS 26.5.2, arm64 (M3, 16 GB). forge 1.5.1, slither 0.11.6, aderyn 0.6.8,
halmos 0.3.3, medusa 1.5.1, echidna 2.3.3. Sandbox at `~/Work/btr/audit/.corpus/sandbox/`, a
throwaway `forge init` pinned to the BTR compile profile (solc 0.8.36 + `via_ir = true` +
optimizer 200). Nothing outside `.corpus/` was touched.

### Ran it, here is the number

| tool | command | measured result |
|---|---|---|
| `slither-mutate` | `slither-mutate src/Toll.sol --test-cmd 'forge test' --timeout 120` | **41 mutants in 2 min**; 3/4 revert-mutants caught, 1/1 comment-mutants, **22/36 tweak-mutants**. That is **14 mutations the test suite does not detect**, which is exactly the P-14 artifact. Note the operator asymmetry: revert and comment mutants were mostly caught, **arithmetic tweaks were not**, and that is the class BTR's pricing law lives in |
| `@openzeppelin/upgrades-core` | `forge build` with `build_info=true`, `extra_output=["storageLayout"]`, then `npx -y @openzeppelin/upgrades-core@latest validate out/build-info` | On a deliberate field reorder: `src/Up.sol:16: Inserted 'owner' ... VaultV1: Deleted 'owner'`, exit FAILED. On the clean pair: PASS. **Negative control passed** |
| forge FFI differential (Sol vs Rust) | `forge test --ffi --match-contract DiffTest --fuzz-runs 2000` | **2,001 runs PASS in 42 s**. With `amount/10 -> amount/9` injected into the Rust side it FAILED with `804 != 893; args=[3.241e19, 5.819e22, 8045]`. **Detects a one-character divergence.** Cost is process-spawn bound at ~21 ms/run, so batch inputs for 10^5 runs |
| Echidna 2.3.3 | `echidna . --contract H --config echidna.yaml` | **20,218 calls in 25 s (~4,800 calls/s)** under via_ir, source-mapped coverage emitted; lcov for `src/Toll.sol` was `DA:5,6,10,11,13,15,16,17` |
| Medusa 1.5.1 | `medusa fuzz` with `compilation.platformConfig.args = ["--foundry-compile-all"]` | **20,000 tx in 3 s at 81,556 calls/s**, 99 branches, lcov + HTML. **The compile step took 1 m 41 s**, twenty times longer than the fuzzing; our recorded command omits it, so a first run reads as a hang. lcov `DA:5,6,11,13,15,16,17` |
| Echidna vs Medusa coverage | same source, same via_ir build | **Both omit lines 12 and 14, which provably execute on every call** (the IR optimizer merged them), and Medusa additionally misses line 10. The deflation is a property of via_ir, not of Echidna |
| slither-mcp | stdio JSON-RPC probe: `initialize` -> `notifications/initialized` -> `tools/list` -> 4 real calls | Verified working: `get_project_overview`, `get_storage_layout`, `get_function_callers` (**inverse call graph**), `get_function_callees`, `find_dead_code`, `export_call_graph`, `analyze_low_level_calls`, `analyze_modifiers`, `analyze_state_variables`, `run_detectors`. **Gotcha: every tool's arguments are wrapped in a `request` object**; a flat `{"path": ...}` fails pydantic validation |
| aderyn mcp | `aderyn mcp stdio` | 7 tools verified. **Gotcha: `aderyn mcp` alone exits**; the `stdio` subcommand is mandatory |
| Solodit / Claudit endpoint | `POST https://solodit.cyfrin.io/api/v1/solodit/findings` | Shape verified. Unkeyed returns `401 {"message":"Missing API key"}`; GET returns `405`. Header is `X-Cyfrin-API-Key`. Free key, no application, **20 requests / 60 s**. **Not run with a real key** |
| gambit | `gambit mutate --filename src/X.sol --solc <svm path>` | **53 mutants in 0.98 s**, no test runner. macOS solc lives under `~/Library/Application Support/svm/`, not `~/.svm` |
| vertigo-rs | `python vertigo.py run` (needs `ast = true`) | **13 mutants, 58 s, 5/13 killed**. Strictly worse than slither-mutate's 41 |
| weasel | `cargo build --release`, `weasel run` | **0.03 s, no compilation** (parser-only, so via_ir and solc version are irrelevant), 196 detectors of which ~27 are security-class |
| solidity-agent-toolkit | `tools/list` probe | 10 tools list fine. Rejected on the regex-fallback disclosure, not on a failure |
| **V1-A** solc `SpillSlotCollisionAcrossMutualRecursion` | `slither . --print call-graph` (exit 0, 68 contracts), 53 dot files, `all_contracts.call-graph.dot` = **991 nodes / 2,248 edges**, then Tarjan SCC over the union graph + a per-file self-loop scan | **REFUTED, bug unreachable.** **Multi-node SCCs: 0.** One self-loop, a false positive (`AccessControl.completeOwnershipHandover` calling `super.` into Solady's `Ownable`, modelled by Slither as a same-name self-edge; a 1-cycle would not satisfy the ">=2 mutually recursive" condition anyway). Corroborated by a 9-hit same-name-call scan, all resolved by hand as overloads, modifiers, or Yul-local helpers. Scope caveat: the compile used `--skip ./test/** ./script/**` |
| **V1-B** `MemoryByteArrayElementDeleteClearsWholeWord` | `FOUNDRY_PROFILE={default,debug,bnb} forge config --json` (the resolved config foundry computes, not a TOML read), plus a grep of every foundry config and the CI workflow | **VERIFIED N/A.** All three profiles resolve to `via_ir = true`, solc 0.8.36, evm prague. `foundry.toml:38-39` records why no non-via_ir profile exists: the pricing libraries do not fit the stack without it. CI sets no `FOUNDRY_PROFILE` and passes no override. The bug's `viaIR: false` condition is never satisfied |
| **V1-C** Solady transient guard off mainnet | read `.deps/solady` at the `foundry.lock` pin, plus `forge remappings`, plus a grep of every `nonReentrant` site | **VERIFIED, BTR-03 upheld, two nuances.** Vendored solady is `v0.1.26` (`acd959aa...`), pinned identically in `dex-evm` and `shared`. `ReentrancyGuardTransient.sol:120-121` defaults `_useTransientReentrancyGuardOnlyOnMainnet()` to `true`; `TransientGuard.sol:14-22` overrides it to `false`, forcing `tstore`/`tload` on every chain. **Nuance 1:** `Router` never inherits it and rolls its own persistent `uint256 _entered`, so the checklist wording should say "Pool and Flash", not "every guarded entrypoint". **Nuance 2:** `FLASH_INFLIGHT` and `TransientCacheLib` are raw assembly with no chainid branch, so they never inherit the conditional behaviour. Counterfactual: removing the override would `sstore` slot `0x8000000000ab143c06`, which collides with nothing but is **invisible to `ArtifactGuards.t.sol`**, since a raw-assembly `sstore` never appears in `storageLayout` |
| **V1-D** linked-library struct layout | read `ArtifactGuards.t.sol` and the freshly built artifacts; grepped `linkReferences` over `test/`, `scripts/`, `script/` | **VERIFIED, no automated detection exists.** `out/{Pricing,PoolLiquidity,PoolConfig,NUQuartic}.json` each have **0 entries in `storageLayout.storage` and 0 in `storageLayout.types`**; only `out/Pool.sol/Pool.json` has 1 and 22. solc emits no `types` map for a stateless library, so **there is no field in a library artifact to compare against Pool's pinned layout**, and no artifact-reading test can be extended to do it. Exactly one grep hit for `linkReferences` across the whole test and script tree, and it is a runbook, not a check. The related failure mode has already happened on Arc (a foreign `PoolLiquidity` build carrying its own baked-in `Pricing`, reverting `ZeroValue()` on every `swapLiability`), and was caught **on chain by a revert**. A struct-offset mismatch would read and write wrong slots silently, with no revert to catch it |

**Disclosed side effect of V1:** `slither .` invokes crytic-compile, which ran `forge clean` and
`forge build --build-info --force` in `dex-evm/`, wiping and rebuilding `out/`. Slither also wrote
53 `*.call-graph.dot` files into the repo root; they were moved to `/tmp/v1/` and the root
re-verified clean. `forge build` was re-run afterwards. No source file was touched and no git
command was run.

### Read about it, not run

Marked so nobody reads a documented claim as a measurement: Certora Prover local build (licence
claim verified from source, **build not attempted**, 7,231 files with JVM + Rust on a 16 GB box);
Kontrol (nix/JVM heavy, README says 30 to 60 min first run); Bitwuzla / Echidna `symExec`;
`fuzz-utils`; the in-test `Upgrades.validateUpgrade` form (only the CLI form was run); Claudit with
a real key; any Foundry MCP; ToB `mewt`; ToB `trailmark` (Solidity parser quality on
DELEGATECALL-linked libraries unknown, and its own docs warn that `tainted` is call-graph
reachability, not dataflow); Paradigm `evmbench` (not cloned, eval spec is in a `frontier-evals`
submodule); Wake, 4naly3er and Heimdall (the evaluation sub-task did not return).

---

## What this sweep did NOT cover

The honest gap list, so the next sweep starts here rather than re-deriving it.

**Corpora present but unmined.**
- Solady `audits/`, 5 PDFs at `2afba69`. The most likely place to find a known issue in a file BTR
  compiles into every implementation.
- CDSecurity `audit reports/`, 71 PDFs at `f3275c4`. No index, no search. `BeezieStableSwap-Audit.pdf`
  is the nearest domain.
- SlowMist `open-report-V2/` at `ce4cbdb`, 917 MB, mostly PDF and zh-cn.
- `crytic/properties` beyond the harness kit: 168 properties, of which 106 are ABDKMath64x64.
- Solodit's 20k+ findings themselves. The MCP was verified but never run with a key, so no sibling-AMM
  prior-art query was actually executed.

**Named in an index and never opened.** From `marchev` and `gonzaloetjo`: `scabench-org/hound`
(knowledge-graph auditor), `JoranHonig/grimoire` (best FP-rejection rate in the Majeur benchmark),
`forefy/.context`, `zerocoolailabs/ZeroSkills` (Slot Sleuth, EVM storage-safety scanner).
`CDSecurity/cdsecurity-skills` was on this list; ingested post-sweep 2026-09-10 (`930cc41`, registry row above).
From TradMod: Sigma Prime "Oracles and Pricing"
(flagged as the highest-value unassigned item, BTR's oracle is homegrown and push-based),
MixBytes "Pitfalls of CREATE/CREATE2/EXTCODESIZE", dacian's dedicated "Signature Replay Attacks"
article, SEAL "Crisis Handbook" and howtomultisig.com.

**Not searched at all.** SmartBugs, SolidiFI, AuditGPT, SmartAuditFlow. A gap, not an absence.
HackerOne / Bugcrowd platform-wide AI-rejection statistics: NOT FOUND.

**Tools evaluated on documentation only.** Certora local build, Kontrol, Bitwuzla, `mewt`,
`trailmark` on Solidity, `evmbench`, Wake, `fuzz-utils`, Claudit with a key. Every one of these is
a claim, not a measurement, until run.

**Calibration never performed.** The single question S1's C-4 and S10's C-3 both raise is whether
our loop beats one strong model asked directly, and nobody has measured it on our own corpus. The
mechanical next step is one lens against ScaBench's curated set (31 projects, 555 vulns, GPT-5
single-file baseline included) or against `evmbench`, compared to the published baseline.

**Surfaces no slice owned.** These are the scope gaps the empirical layer says matter most, and the
sweep found sources for them without reviewing the surfaces themselves: `keepers/` (Rust), the NXR
wire producer, `back/` gateway and quote, the deploy and governance scripts, the SDK and front quote
path, `front/` as a Preact SPA on a public origin, `content/` (a push to `main` is live in under 5 s
with no build and no review gate), the registry, keeper hosts, RPC endpoints and the sequencer.
GOV-08 already records that a deploy script shipped kappa = 0 on 28 of 37 legs, and that class of bug
lives outside `dex/evm/src`, which is where every round has looked.

**Sub-sweeps deliberately deferred.** A larger Code4rena sweep and a Sherlock cross-check (their
`-judging` repos label duplicate groups on issues) would strengthen the solo-find backbone; both are
mechanical and were not done. The Bancor withdrawal formulas were not recovered from any source. The
`@arsen_bt` thread was not recovered and was deliberately not reconstructed.
