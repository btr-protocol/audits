<!-- GENERATED by scripts/publish-public.sh from toolkit/SKILL.md. Do not edit here. -->
...
- Re-passes are **delta-only**: round N+1 re-reads only (a) sites with findings in round N,
  (b) files no lens logged in `archive/2026-09-15/EYES.md`, (c) every fix diff. A clean file is not re-read.
- **Carry the path, withhold the verdict.** A re-pass agent gets what round N *explored* ("traced
  the toll through `_covToll`; never opened the σ path"), never what it *concluded*: a verdict in the
...
file:line in `archive/2026-09-15/EYES.md`:
...
family: a same-family multi-lens round is one methodology with ten remits. Every `archive/2026-09-15/EYES.md` row
